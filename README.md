...
- `@kontourai/ui/react` — class-driven React primitives: display (`Badge`, `Button`, `Panel`, `Metric`, `Progress`, `Skeleton`, `Spinner`, `StatusBadge`, `StatusBar`, `Topbar`, `Empty`, `ProductIcon`) and form controls (`Field`, `Input`, `Textarea`, `Select`, `Checkbox`, `Toggle`) that read the token contract.
